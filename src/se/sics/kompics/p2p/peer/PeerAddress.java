...
public final class PeerAddress extends OverlayAddress implements Comparable<PeerAddress> {
	...
	//added 11/16
	public final int largestID = (int) Math.pow(2, 13);
...
//-------------------------------------------------------------------
	...
	//added 11/16
	public int compareByPeerIdTo(PeerAddress that)
	{
		int subtract = that.peerId.intValue() - this.peerId.intValue() ;
		...
		if(subtract<(largestID/2))
			return subtract;
		else
			return subtract-largestID;
	}
	
	//added 11/16
	public int compareByPeerIdTo(int that)
	{
		int subtract = that - this.peerId.intValue() ;
